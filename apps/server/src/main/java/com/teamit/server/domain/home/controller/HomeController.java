...
public class HomeController {
    ...
    @Operation(summary = "나의 실시간 매칭 현황", description = "받은 제안 수(PENDING)와 지원한 팀 수를 반환합니다.")
    @GetMapping("/matching-status")
    public ApiResponse<MatchingStatusResponse> getMatchingStatus(
            @LoginUser CustomUserDetails userDetails) {
        MatchingStatusResponse response = homeService.getMatchingStatus(userDetails.getUserId());
