...
## 프론트 개발 방식
1. **이슈 먼저** → GitHub Issues에서 할 일 등록
2. **브랜치 파기** → `feature/작업이름-fe`
3. **컨텍스트 주기** → 아래 세 가지를 항상 같이 줘:
    - `docs/api-spec.md`에서 해당 화면 API 부분
    - 와이어프레임 이미지
    - 구현할 화면 설명
4. **더미 데이터로 UI 구현** → API 연결은 나중에
5. **커밋 → PR → 리뷰 → 머지**

### Claude Code 명령어 템플릿
```
나는 티밋 앱 프론트엔드 개발 중이야.
React Native + Expo 사용해.
아래 API 명세서랑 와이어프레임 기반으로 [화면명] 화면 만들어줘.
지금은 API 연결 없이 더미 데이터로 UI만 구현해줘.
나중에 API 연결할 부분은 주석으로 표시해줘.

[api-spec.md에서 해당 API 붙여넣기]
[와이어프레임 이미지 첨부]
```
